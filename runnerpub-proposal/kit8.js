...
module.exports = function make(p) {
  ...
  function footer(s, onDark, rightDark, leftDark, onBlue){
    const rd = onDark||rightDark, ld = onDark||leftDark;
    hr(s, SAFE.x, Y.footRule.y, SAFE.w, onBlue?'5A8BF5':((onDark||leftDark)?C.RULED:C.RULE));
    const ft = region('foot', SAFE.x, Y.foot.y, SAFE.w, Y.foot.h);
    const fl = ld ? {x:SAFE.x, w:3.14} : span(0,8), fr = span(10,2);
    tx(s,ft,{x:fl.x,y:Y.foot.y+0.02,w:fl.w,h:0.22},
      '주식회사 내일사장  →  러너스튜디오(주) 러너펍 가맹본부',T.foot,
      {color:onBlue?'C9DAFF':(ld?C.MUTED:C.MUTE), valign:'middle'});
    tx(s,ft,{x:fr.x,y:Y.foot.y+0.02,w:fr.w,h:0.22},
      `${String(FOOTNO).padStart(2,'0')} / 15`,T.foot,
      {color:(onBlue||rd)?C.WHITE:C.NAVY, bold:true, align:'right', valign:'middle'});
  }
  /** 영문 챕터 앵커 */
  ...
  function bandSentence(s,str){                     // 문장형 — 파란 순색 풀블리드 띠
    setBand('문장형');
    rect(s,BLEED,C.BLUE);
    const r=region('band',SAFE.x,Y.band.y,SAFE.w,Y.band.h);
    tx(s,r,{x:SAFE.x,y:Y.band.y+0.06,w:SAFE.w,h:0.40},runsOnBlue(str),T.bandtx,{valign:'middle'});
