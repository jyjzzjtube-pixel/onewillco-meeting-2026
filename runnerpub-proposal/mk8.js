/* 러너펍 가맹 개설 영업 위임 제안 — 주식회사 내일사장
   v8 · 내일사장 IR 문법 이식 — 흰 지면이 주, 다크는 좌측 세로 밴드, 결론은 파란 띠. 15장.
...
const CUT6 = G.colX(7)-G.GUT/2;      // 7.671 — 우측 다크 폭을 IR 비율로 축소
const BAND = 4.10;                   // 챕터 표지 좌측 다크 세로 밴드
const CUT5 = 4.90;                   // 좌측 다크 분할 폭 — IR 비율
const CUT7 = 6.30;                   // Close 좌측 다크 폭
...
/* ══════════════ 01 표지 — 순색 네이비 ══════════════ */
{
  ...
  kpi(s,F,{x:RX,y:4.40,w:RW,h:0.90},'1,000','만원','가맹계약 1건당 성공보수 · VAT 별도',
    {size:48,hi:true,hiW:3.30,dark:true});
...
/* ══════════════ 02 01 Problem — 좌측 다크 밴드 + 흰 지면 ══════════════ */
{
  const s=p.addSlide(); bg(s,C.WHITE);
  splitPanel(s,'left',BAND,C.NAVY);
  skyline(s,0,G.H-1.05,BAND,1.05);
  frame(s,2,'01',false,true,false,true);
  const HB={x:BAND+0.42,w:RIGHT-(BAND+0.42)};
  anchor(s,'Problem',false,HB);
  head(s,'창업자는 [판단 근거가 없어서] 멈춥니다',false,false,HB);
  lead(s,'홀덤펍 창업을 고민하는 사람이 실제로 지나는 경로입니다.',false,HB);

  const B=BODY('full');
  const L=region('pL',SAFE.x,B.y,BAND-SAFE.x-0.42,B.h);
  const R=region('pR',HB.x,B.y,HB.w,B.h);

  pill(s,L,L.x,L.y,'PROBLEM',C.RED,null,1.18);
  kpi(s,L,{x:L.x,y:L.y+0.70,w:L.w,h:1.10},'3','주체','창업자가 만나는 이해관계자',
    {size:56,dark:true,color:C.YEL,unitColor:C.MUTED});
  ['양도자','중개사','컨설팅 업체'].forEach((v,i)=>{
    const y=L.y+2.00+i*0.42;
    rrect(s,at(L,{x:L.x,y,w:L.w,h:0.34},{kind:'chip'}),C.NAVY2,0.06);
    tx(s,L,{x:L.x+0.14,y,w:L.w-0.28,h:0.34},v,T.body,{size:10.5,color:C.INK_ON,valign:'middle'});
  });
  tx(s,L,{x:L.x,y:L.y+3.20,w:L.w,h:0.26},'셋 다 창업자 편이 아닙니다.',T.cardtx,
    {size:9.8,color:C.MUTED,valign:'middle'});

  sectionBar(s,R,R.x,R.y,R.w,'주요 창업 경로');
  [['포털사이트 검색','컨설턴트 매물 추천','브랜드 정보는 컨설턴트가 준다'],
   ['지인 · 커뮤니티','공인중개사 사무소 방문','매물 정보는 중개사가 준다']].forEach(([a,b,tail],ri)=>{
    const y=R.y+0.50+ri*0.48;
    rrect(s,at(R,{x:R.x,y,w:2.32,h:0.38},{kind:'chip'}),C.BLUEBG,0.06);
    tx(s,R,{x:R.x+0.14,y,w:2.04,h:0.38},a,T.body,{size:10.5,color:C.NAVY,valign:'middle'});
    icon(s,R,R.x+2.42,y+0.07,0.24,'arrow',false);
    rrect(s,at(R,{x:R.x+2.76,y,w:2.72,h:0.38},{kind:'chip'}),C.BLUEBG,0.06);
    tx(s,R,{x:R.x+2.90,y,w:2.44,h:0.38},b,T.body,{size:10.5,color:C.NAVY,valign:'middle'});
    icon(s,R,R.x+5.58,y+0.07,0.24,'arrow',false);
    rrect(s,at(R,{x:R.x+5.92,y,w:0.96,h:0.38},{kind:'chip'}),C.RED,0.06);
    tx(s,R,{x:R.x+6.00,y,w:0.80,h:0.38},'계약',T.body,
      {size:10.5,bold:true,color:C.WHITE,align:'center',valign:'middle'});
    tx(s,R,{x:R.x+7.02,y,w:R.w-7.02,h:0.38},tail,T.note,{size:8.5,color:C.MUTE,valign:'middle'});
  });

  const tbTop=R.y+1.62;
  sectionBar(s,R,R.x,tbTop,R.w,'이해관계자별 목적과 창업자가 받는 정보');
  const tb=region('tb',R.x,tbTop+0.46,R.w,B.h-(tbTop-R.y)-0.46);
  { const rh=(tb.h-0.38)/3;
    for(let i=1;i<3;i+=2) rect(s,at(tb,{x:tb.x,y:tb.y+0.38+rh*i,w:tb.w,h:rh},{kind:'row'}),C.BLUEBG0); }
  table(s,tb,[{h:'구분',w:1.60},{h:'거래 목적',w:2.92},{h:'창업자가 받는 정보',w:tb.w-4.52}],[
    ['양도자','높은 권리금 수취 · 빠른 거래 종결',{v:'영업이익이 과장된 정보',c:C.RED}],
    ['중개사','중개 수수료 수취',{v:'인수 후 매출 · 손익과 무관한 정보',c:C.RED}],
    ['컨설팅 업체','높은 수수료 수취',{v:'검증되지 않은 매물 브리핑',c:C.RED}],
  ],{rh:(tb.h-0.38)/3});
...
{
  const s=p.addSlide(); bg(s,C.WHITE);
  rect(s,{x:CUT6,y:0,w:G.W-CUT6,h:G.H},C.BLUEBG);
  frame(s,3,'01');
  ...
  tx(s,R,{x:R.x,y:R.y+0.44,w:R.w,h:0.34},'계약 가능한 창업자 모수',T.sub,
    {size:19,color:C.NAVY,valign:'middle'});
  kpi(s,R,{x:R.x,y:R.y+1.04,w:R.w,h:1.04},'1.5','배','앱 리뉴얼 후 플랫폼 이용자',
    {size:52,color:C.BLUE,unitColor:C.NAVY});
  const pf=P('p11_pub.png'), pAv=R.h-2.16, pw=Math.min(R.w,pAv*1.778), ph=pw/1.778;
  s.addImage({path:pf, ...img(R,{x:R.x+(R.w-pw)/2,y:R.y+2.16+(pAv-ph)/2,w:pw,h:ph},pf)});
  note(s,'※ IT비즈뉴스 2024.7.4 보도 기준',false,{x:RX,w:R.w});

  bandFig(s,'계약 이전 구간에서 내일사장이 이미 확보한 상담 DB','5,114','명');
  s.addNotes('러너펍은 개설 이후 구조가 완비돼 있습니다. 남은 건 창업자 접점입니다.');
}

/* ══════════════ 04 02 Solution — 좌측 다크 밴드 + 흰 지면 ══════════════ */
{
  const s=p.addSlide(); bg(s,C.WHITE);
  splitPanel(s,'left',BAND,C.NAVY);
  skyline(s,0,G.H-1.05,BAND,1.05);
  frame(s,4,'02',false,false,false,true);
  const HB={x:BAND+0.42,w:RIGHT-(BAND+0.42)};
  anchor(s,'Solution',false,HB);
  head(s,'이미 확보된 [창업 수요와 제휴 네트워크]',false,false,HB);
  lead(s,'모수를 새로 만드실 필요가 없습니다. 이번 제안 이전에 확보돼 있는 값입니다.',false,HB);

  const B=BODY();
  const L=region('sL',SAFE.x,B.y,BAND-SAFE.x-0.42,B.h);
  const R=region('sR',HB.x,B.y,HB.w,B.h);

  kpi(s,L,{x:L.x,y:L.y+0.34,w:L.w,h:1.10},'5,114','명','예비창업자 상담 DB',
    {size:48,dark:true,color:C.YEL,unitColor:C.MUTED});
  tx(s,L,{x:L.x,y:L.y+1.56,w:L.w,h:0.62},
    '러너펍에 맞는 사람을 이 안에서 먼저 찾습니다.',T.body,{size:10.5,color:C.MUTED});
  [['app','앱 누적 다운로드','100,000 건'],
   ['users','월간 활성 이용자','50,000 명'],
   ['chart','누적 매물 거래 규모','1,600 억원']].forEach(([ic,k,v],i)=>{
    const y=L.y+2.14+i*0.36;
    icon(s,L,L.x,y+0.03,0.20,ic,true);
    tx(s,L,{x:L.x+0.26,y,w:1.28,h:0.26},k,T.note,{size:8.8,color:C.MUTED,valign:'middle'});
    tx(s,L,{x:L.x+L.w-1.32,y,w:1.32,h:0.26},v,T.glabel,
      {size:10,color:C.INK_ON,align:'right',valign:'middle'});
  });

  const Rw=G.rows(R,[['g1',1.26,'fix'],['g2',0.14,'fix'],['g3',1.10,'fix'],['g4',0.10,'fix'],['lg',0.54,'fix']]);
  sectionBar(s,Rw.g1,Rw.g1.x,Rw.g1.y,Rw.g1.w,'앱 누적 다운로드 추이');
  [['2023',3],['2024',8],['2026',10]].forEach(([yy,v],i)=>{
    const y=Rw.g1.y+0.48+i*0.26;
    tx(s,Rw.g1,{x:Rw.g1.x,y,w:0.62,h:0.24},yy,T.note,{size:9,color:C.MUTE,valign:'middle'});
    bar(s,Rw.g1,Rw.g1.x+0.68,y+0.05,Rw.g1.w-1.50,0.14,v/10,C.BLUE,C.BLUEBG);
    tx(s,Rw.g1,{x:Rw.g1.x+Rw.g1.w-0.76,y,w:0.76,h:0.24},`${v}만`,T.glabel,
      {size:10,color:C.BLUE,align:'right',valign:'middle'});
  });
  sectionBar(s,Rw.g3,Rw.g3.x,Rw.g3.y,Rw.g3.w,'인증 서비스 도입 효과');
  [['자영업자 1년 생존률','67 %','99 %','+32%p'],
   ['양도양수 성공 비율','47 %','80 %','+33%p']].forEach(([k,a,b,d],i)=>{
    const y=Rw.g3.y+0.46+i*0.32;
    tx(s,Rw.g3,{x:Rw.g3.x,y,w:2.30,h:0.26},k,T.body,{size:10.5,color:C.NAVY,valign:'middle'});
    s.addText([{text:a,options:{fontSize:11,color:C.MUTE}},
               {text:'  →  ',options:{fontSize:10,color:C.MUTE}},
               {text:b,options:{fontSize:22,bold:true,color:C.BLUE}}],
      t({...at(Rw.g3,{x:Rw.g3.x+2.40,y:y-0.06,w:Rw.g3.w-3.40,h:0.38},{kind:'fig',pt:22}),valign:'middle'}));
    tri(s,Rw.g3,Rw.g3.x+Rw.g3.w-0.86,y+0.04,0.15,0.18,C.RED);
    tx(s,Rw.g3,{x:Rw.g3.x+Rw.g3.w-0.66,y,w:0.66,h:0.26},d,T.glabel,
      {size:9.5,color:C.RED,align:'right',valign:'middle'});
  });

  rrect(s,at(Rw.lg,{x:Rw.lg.x,y:Rw.lg.y,w:Rw.lg.w,h:Rw.lg.h},{kind:'card'}),C.BLUEBG0,0.06);
  const KEYS=['spc','samsung','kfa','barogo','saramin','forbes'];
  const LG=KEYS.map(k=>{const f=P(`lg_${k}.png`),a=G.imgAspect(f);
    let w=Math.min(0.26*a,1.42); return {f,w,h:w/a};});
  const gapW=(Rw.lg.w-0.56-LG.reduce((t,l)=>t+l.w,0))/(LG.length-1);
  let lx=Rw.lg.x+0.28, mid=Rw.lg.y+Rw.lg.h/2;
  LG.forEach(l=>{ s.addImage({path:l.f, ...img(Rw.lg,{x:lx,y:mid-l.h/2,w:l.w,h:l.h},l.f)}); lx+=l.w+gapW; });

  bandFig(s,'모수 확보를 위해 본사가 쓰실 시간과 비용','0','원',{labelBox:{x:BAND+0.42,w:5.60}});
  note(s,'※ 플랫폼 지표는 내일사장 누적 기준(2026.07)이며, 생존률·성공비율은 내일사장 인증 서비스 이용 건 기준 자체 집계값입니다.',
    false,{x:BAND+0.42,w:RIGHT-(BAND+0.42)});
...
/* ══════════════ 10 03 Terms — 좌측 다크 밴드 + 흰 지면 ══════════════ */
{
  const s=p.addSlide(); bg(s,C.WHITE);
  splitPanel(s,'left',BAND,C.NAVY);
  skyline(s,0,G.H-1.05,BAND,1.05);
  frame(s,10,'03',false,false,false,true);
  const HB={x:BAND+0.42,w:RIGHT-(BAND+0.42)};
  anchor(s,'Terms',false,HB);
  head(s,'착수금도 광고비도 [청구하지 않습니다]',false,false,HB);
  lead(s,'청구는 가맹계약이 체결되고 가맹비 입금이 확인된 뒤에만 발생합니다.',false,HB);

  const B=BODY();
  const L=region('tL',SAFE.x,B.y,BAND-SAFE.x-0.42,B.h);
  const R=region('tR',HB.x,B.y,HB.w,B.h);

  tx(s,L,{x:L.x,y:L.y+0.10,w:L.w,h:0.26},'계약 전까지 본사 지출',T.label,
    {color:C.MUTED,valign:'middle'});
  rect(s,at(L,{x:L.x,y:L.y+0.50,w:2.30,h:0.94},{kind:'hl'}),C.YEL);
  s.addText([{text:'0',options:{fontSize:64,bold:true,color:C.NAVY,charSpacing:-2}},
    ...
    t({...at(L,{x:L.x+0.18,y:L.y+0.36,w:2.3,h:1.22},{kind:'fig',pt:64}),valign:'middle'}));
  tx(s,L,{x:L.x,y:L.y+1.72,w:L.w,h:0.92},
    '상담 · 분석 · 브리핑과 창업마케팅에 들어간 비용은 계약이 성사되지 않아도 전부 내일사장이 부담합니다.',
    T.body,{size:10.5,color:C.MUTED});
  icon(s,L,L.x,L.y+2.76,0.26,'won',true);
  tx(s,L,{x:L.x+0.36,y:L.y+2.77,w:L.w-0.36,h:0.24},'성공보수는 부가가치세 별도',T.note,
    {size:9,color:C.MUTED,valign:'middle'});

  sectionBar(s,R,R.x,R.y,R.w,'비용 발생 시점과 금액');
  const tb=region('tt',R.x,R.y+0.48,R.w,B.h-0.48);
  { const rh=(tb.h-0.38)/5;
    for(let i=1;i<5;i+=2) rect(s,at(tb,{x:tb.x,y:tb.y+0.38+rh*i,w:tb.w,h:rh},{kind:'row'}),C.BLUEBG0); }
  table(s,tb,[{h:'항목',w:1.90},{h:'발생 시점',w:tb.w-4.10},{h:'금액',w:2.20,a:'right'}],[
    ['착수금','해당 없음',{v:'0 원',b:true,c:C.BLUE}],
    ['월 고정비','해당 없음',{v:'0 원',b:true,c:C.BLUE}],
    ['광고비','가맹 개설 영업 대가로는 청구하지 않습니다',{v:'0 원',b:true,c:C.BLUE}],
    [{v:'성공보수',b:true},'가맹계약 체결 및 가맹비 입금 완료 후',{v:'1,000 만원',b:true,c:C.RED}],
    ['해제 · 환불 시','본사 정책에 맞춰 착수 전 협의',{v:'협의',c:C.MUTE}],
  ],{rh:(tb.h-0.38)/5});

  bandFig(s,'계약 · 입금 완료 전 내일사장에 지급하는 현금','0','원',{labelBox:{x:BAND+0.42,w:5.60}});
...
{
  const s=p.addSlide(); bg(s,C.WHITE);
  rect(s,{x:CUT6,y:0,w:G.W-CUT6,h:G.H},C.BLUEBG0);
  frame(s,12,'03');
  ...
  tx(s,R,{x:R.x,y:R.y,w:R.w,h:0.26},'1개점 누적 본사 수익',T.label,{color:C.MUTE,valign:'middle'});
  hr(s,R.x,R.y+0.34,R.w,C.NAVY,0.016);
  [['12개월',2300],['24개월',4100],['36개월',5900]].forEach(([k,v],i)=>{
    const y=R.y+0.50+i*0.42;
    stepBadge(s,R,R.x,y+0.02,0.24,i+1);
    tx(s,R,{x:R.x+0.32,y,w:0.92,h:0.28},k,T.body,{size:11,color:C.NAVY,valign:'middle'});
    bar(s,R,R.x+1.30,y+0.05,R.w-3.24,0.18,v/5900,i===2?C.BLUE:C.BLUEP,C.WHITE);
    tx(s,R,{x:R.x+R.w-1.66,y,w:1.66,h:0.28},v.toLocaleString()+' 만원',T.glabel,
      {color:i===2?C.BLUE:C.NAVY,align:'right',valign:'middle'});
  });
  hr(s,R.x,R.y+1.86,R.w,C.RULE);
  tx(s,R,{x:R.x,y:R.y+1.98,w:2.20,h:0.24},'출점 규모별 36개월 누적',T.label,{color:C.MUTE,valign:'middle'});
  tx(s,R,{x:R.x+R.w-1.10,y:R.y+1.98,w:1.10,h:0.24},'단위 : 만원',T.note,
    {size:8.5,color:C.MUTE,align:'right',valign:'middle'});
...
/* ══════════════ 13 04 Governance — 좌측 다크 밴드 + 흰 지면 ══════════════ */
{
  const s=p.addSlide(); bg(s,C.WHITE);
  splitPanel(s,'left',BAND,C.NAVY);
  skyline(s,0,G.H-1.05,BAND,1.05);
  frame(s,13,'04',false,true,false,true);
  const HB={x:BAND+0.42,w:RIGHT-(BAND+0.42)};
  anchor(s,'Governance',false,HB);
  head(s,'준법 영업을 [사람이 아니라 시스템]이 막습니다',false,false,HB);
  lead(s,'가맹사업법 위반은 본사 리스크입니다. 사람이 달력을 세지 않게 만들었습니다.',false,HB);

  const B=BODY('full');
  const L=region('gvL',SAFE.x,B.y,BAND-SAFE.x-0.42,B.h);
  const R=region('gvR',HB.x,B.y,HB.w,B.h);

  tx(s,L,{x:L.x,y:L.y+0.08,w:L.w,h:0.26},'법정 숙려기간',T.label,{color:C.MUTED,valign:'middle'});
  rect(s,at(L,{x:L.x,y:L.y+0.48,w:2.24,h:0.92},{kind:'hl'}),C.YEL);
  s.addText([{text:'14',options:{fontSize:62,bold:true,color:C.NAVY,charSpacing:-2}},
    ...
    t({...at(L,{x:L.x+0.18,y:L.y+0.34,w:2.24,h:1.20},{kind:'fig',pt:62}),valign:'middle'}));
  icon(s,L,L.x,L.y+1.72,0.28,'lock',true);
  tx(s,L,{x:L.x,y:L.y+2.14,w:L.w,h:1.10},
    '정보공개서 제공 후 14일이 지나기 전에는 시스템에서 전자계약 버튼이 잠깁니다. 기간 경과 전 계약이 구조적으로 불가능합니다.',
    T.body,{size:10.5,color:C.MUTED});

  sectionBar(s,R,R.x,R.y,R.w,'항목별 준법 기준');
  const gt=region('gvt',R.x+0.32,R.y+0.48,R.w-0.32,B.h-0.48);
  { const rh=(gt.h-0.38)/5;
    for(let i=1;i<5;i+=2) rect(s,at(R,{x:R.x,y:gt.y+0.38+rh*i,w:R.w,h:rh},{kind:'row'}),C.BLUEBG0); }
  ...
  table(s,gt,[{h:'항목',w:2.50},{h:'기준',w:gt.w-2.50}],
    GV.map(([k,v])=>[{v:k,b:true},v]),{rh:(gt.h-0.38)/5});
  GV.forEach((_,i)=>{
    const rh=(gt.h-0.38)/5;
    check(s,R,R.x+0.02,gt.y+0.42+rh*i+(rh-0.20)/2,0.20);
  });
...
/* ══════════════ 15 Close — 좌우분할 · 좌측 다크 ══════════════ */
{
  ...
  [['전속 · 비전속','전속으로 하실지, 타 대행사와 병행하실지'],
   ...
   ['인테리어 시공','내일사장 시공 진행 여부 · 추가 수익 분배는 별도 협의']].forEach(([k,v],i)=>{
    const y=L.y+i*0.60;
    if(i===0) rect(s,at(L,{x:L.x,y,w:0.06,h:0.30},{kind:'hl'}),C.YEL);
    stepBadge(s,L,L.x+0.16,y+0.01,0.26,i+1,true);
    tx(s,L,{x:L.x+0.54,y,w:L.w-0.54,h:0.26},k,T.sub,{size:12.5,color:C.INK_ON,valign:'middle'});
    tx(s,L,{x:L.x+0.54,y:y+0.25,w:L.w-0.54,h:0.24},v,T.cardtx,{size:9.8,color:C.MUTED});
    hr(s,L.x,y+0.52,L.w,C.RULED);
  });
  tx(s,L,{x:L.x,y:L.y+3.10,w:L.w,h:0.28},
...
/* ═══ 저장 ═══ */
const OUT='/tmp/claude-0/-home-user-onewillco-meeting-2026/e45e1d59-bb0f-5971-8866-2e14a767d632/scratchpad/v4/runnerpub_v8.pptx';
const rep=G.report('/tmp/claude-0/-home-user-onewillco-meeting-2026/e45e1d59-bb0f-5971-8866-2e14a767d632/scratchpad/v4/layout_report_v8.txt');
